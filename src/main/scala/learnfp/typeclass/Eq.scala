...
object EqInstances {
  ...
  implicit def listEqInstance[A](implicit eqt:Eq[A]): Eq[List[A]] = new Eq[List[A]] {
    override def eq(lhs: List[A], rhs: List[A]): Boolean = lhs.length == rhs.length && lhs.zip(rhs).forall{
      case (l, r) => eqt.eq(l, r)
    }
  }
}
